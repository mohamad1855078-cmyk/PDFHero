...
import { FileText, X, Upload, FileType } from 'lucide-react';
...
export default function WordToPDF() {
  ...
  const handleConvert = async () => {
    ...
    try {
      ...
      const response = await fetch('/api/pdf/from-word', {
        method: 'POST',
        body: formData,
      });
      ...
      toast({
        title: t('tool.wordToPdf.success'),
        description: t('tool.wordToPdf.successDesc'),
      });
    } catch (error: any) {
      console.error('Error converting document:', error);
      toast({
        title: t('tool.edit.error'),
        description: error.message || t('tool.wordToPdf.errorDesc'),
        variant: "destructive"
      });
    } finally {
      ...
      {!file ? (
        <div className="py-12">
          <div
            ...
            data-testid="dropzone-word"
          >
            <input {...getInputProps()} />
            <div className="flex flex-col items-center gap-4">
              <div className={cn(
                "w-16 h-16 rounded-full flex items-center justify-center transition-colors",
                isDragActive ? "bg-[#11A05C]/10 text-[#11A05C]" : "bg-blue-100 dark:bg-blue-900 text-blue-600 dark:text-blue-400"
              )}>
                <Upload className="w-8 h-8" />
              </div>
              <div>
                <p className="text-lg font-medium mb-1">
                  {isDragActive ? t('tool.wordToPdf.dropHere') : t('tool.wordToPdf.uploadDesc')}
                </p>
                <p className="text-sm text-muted-foreground">
                  {t('tool.wordToPdf.supportedFormats')}
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="space-y-8 max-w-2xl mx-auto">
          ...
          <div className="bg-blue-50 dark:bg-blue-950/30 border border-blue-100 dark:border-blue-900 rounded-2xl p-6 text-center">
             ...
             <h3 className="text-lg font-semibold mb-2">{t('tool.wordToPdf.ready')}</h3>
             <p className="text-muted-foreground">
               {t('tool.wordToPdf.readyDesc')}
             </p>
          </div>
        </div>
      )}
